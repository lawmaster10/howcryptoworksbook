...
The most striking real-world test of this mechanism came in mid-2021, when China banned cryptocurrency mining outright. Chinese miners had accounted for roughly 44% of global hash rate at the time, and as recently as 2019 had represented as much as 75%. Within weeks of the ban, that share fell to essentially zero as miners shut down or scrambled to relocate equipment abroad. Network hash rate dropped sharply and blocks slowed temporarily. But the difficulty retarget did exactly what it was designed to do: difficulty fell to reflect the reduced competition, blocks returned to their 10-minute average, and the network continued operating without interruption. By August 2021, the United States had emerged as the new leading mining country, followed by Kazakhstan and Russia. A crackdown framed as an existential threat instead became the clearest demonstration of Bitcoin mining's geographic resilience: no single government controls enough of the global hash rate to permanently disable the network.
...
Having established how Bitcoin's network operates and maintains security, we can now explore how these foundational mechanisms enable higher-layer innovation. This section examines two distinct categories of extension: Layer 2 systems that expand the base layer's capabilities (faster payments, richer programmability, enhanced privacy), and data layer applications that leverage Bitcoin's immutability for on-chain use cases like digital artifacts and tokens. While these serve fundamentally different purposes, both demonstrate how a simple, constrained base layer can enable sophisticated higher-layer functionality.

### L2 Classification and Trust Models

In the Ethereum ecosystem, "Layer 2" typically refers to rollups that scale transaction throughput. In Bitcoin, the term is broader: an L2 is any system that extends the base layer's capabilities, whether that means faster payments, richer programmability, enhanced privacy, or new contract functionality. Scaling is one motivation, but not the only one.

What unifies these systems is a shared design goal: minimizing the trust a user must place in anyone other than Bitcoin's base layer itself. The gold standard is that a user's safety, specifically their ability to recover their BTC back to L1, depends only on actions they (or someone they trust) can take directly on L1 using Bitcoin's own consensus rules. The closer an L2 gets to this ideal, the stronger its security guarantees. The further it falls, the more it resembles a custodial service with extra steps.

Evaluating where a given system sits on this spectrum requires understanding what Bitcoin's base layer can and cannot enforce today. The fundamental limitation is that while Bitcoin Script can verify signatures and basic spending conditions, it cannot enforce complex constraints on future transactions or verify claims about external state.

This matters for the definitional challenge of what constitutes a genuine L2: a system that inherits the security properties of its base layer without introducing additional trust assumptions. True L2s allow users to unilaterally exit back to the main chain using only cryptographic proofs, no permission needed from any third party. The base layer's consensus mechanism can directly adjudicate disputes and enforce the L2's rules. Most current Bitcoin L2 solutions, however, are more accurately described as sidechains or federated networks with Bitcoin bridges, since they require users to rely on external validators beyond Bitcoin's own consensus.
...
While covenant opcodes might take years to activate, BitVM takes a different approach. It works with Bitcoin exactly as it exists today, without any upgrades needed. There are many variants (BitVM, BitVM2, BitVM3) that differ significantly in their mechanics, but the core ideas are shared. The designs are still evolving quickly (one version from July 2025 had to be pulled back after researchers found a security flaw) and these approaches remain experimental rather than production-ready.

To understand BitVM's trust model, it helps to separate two distinct assumptions:

The first is **operator trust**. Today, without covenants, BitVM-based systems require a committee of operators or signers to facilitate withdrawals and other protocol operations. The system provides a 1-of-n safety guarantee: the protocol works as intended as long as at least one committee member is honest. This is a significant improvement over federation models that require trusting a majority, but it's still a trust assumption rooted in the committee's composition. Covenants could potentially eliminate this assumption entirely, reducing it to 0-of-n by letting Bitcoin's own consensus rules enforce the protocol directly, with no committee needed.

The second is **optimistic verification**. Even with covenants, if you want to avoid running the full L2 computation directly on Bitcoin (which would be prohibitively expensive), you need an optimistic approach: assume the operator's claims about the L2 state are correct, but allow anyone to challenge fraudulent claims on the Bitcoin blockchain within a dispute window (typically days to two weeks). If the challenge succeeds, the dishonest operator loses their collateral. Crucially, the set of potential challengers here is much broader than the operator committee: in principle, anyone in the world can raise the alarm. This assumption is inherent to the optimistic model and persists even with covenants. You could remove it by running the computation on Bitcoin directly, but that's a cost-versus-trust tradeoff.

So BitVM involves two layers of trust minimization. Covenants can address the first (eliminating reliance on an honest committee member), but the second (someone must challenge fraud) is a deliberate design choice to avoid on-chain computation costs. Today's federation-based systems, by contrast, ask you to trust that most signers will be honest, and covenant-enabled systems would move that enforcement into Bitcoin's consensus rules themselves, making invalid withdrawals literally impossible regardless of what any federation tries to do.

Whether Bitcoin will actually adopt these changes, and when, is still an open question in the development community. As of early 2026, the first BitVM-based L2s have begun launching (notably Citrea), bringing these trust models from theory into practice, though they remain early-stage.

### Lightning Network

Bitcoin's base layer is optimized for high-assurance settlement, which makes small everyday payments impractical. High fees, limited block space, and 10-minute average block times mean buying coffee with an on-chain transaction doesn't make sense. The Lightning Network attempts to solve this by moving small, frequent payments off the main blockchain.
...
Lightning's biggest challenge is **liquidity**. Users can only send payments if there's enough balance on their side of a channel (outbound capacity), and can only receive if the other side has enough room (inbound capacity). Having liquidity in the right place at the right time remains a fundamental constraint despite ongoing technical improvements like multi-path payments and liquidity services.

Beyond liquidity, Lightning requires users to be online to receive payments, demands active channel management, and ties up capital in channels that become more expensive to open and close when base layer fees rise. These operational demands are difficult for non-technical users.
...
### Other L2 and L1 Applications

Lightning is the most established Bitcoin L2, but the design space is broader. **Ark** is a protocol that takes a different approach to off-chain payments. Instead of persistent two-party channels, Ark uses an operator (called an Ark Service Provider) who periodically settles batches of virtual UTXOs on-chain. Users can transact off-chain through the operator without needing to manage channels or liquidity themselves. The tradeoff is that the operator must be online and solvent, and users must come on-chain periodically to refresh their virtual UTXOs or risk the operator reclaiming the funds. Ark represents a different point on the usability-versus-trust spectrum: simpler UX than Lightning's channel management, but with its own liveness assumptions.

On the L1 application layer, **Discrete Log Contracts (DLCs)** enable conditional Bitcoin payments based on real-world outcomes without requiring complex smart contracts. DLCs use cryptographic techniques to create contracts where payouts depend on data published by an oracle (a data feed signer), but the oracle never learns what contract it's serving or who the participants are. This has enabled Bitcoin-native financial applications like collateralized lending, where borrowers lock BTC in a DLC and receive a loan, with the contract automatically handling liquidation or repayment based on price data. These applications expand Bitcoin's utility beyond simple transfers while remaining rooted in L1 transactions.

### Beyond Payments: Bitcoin as a Data Layer

Having explored L2 systems and L1 applications, we now turn to an entirely different category of Bitcoin extension. While Lightning and L2s attempt to reduce on-chain activity, Ordinals and inscription-based systems embrace it, using Bitcoin's immutability and censorship resistance to anchor arbitrary data permanently on the blockchain. This represents a philosophical shift from "Bitcoin as payment rail" to "Bitcoin as permanent storage layer."
...
An inscribed satoshi functions like a Bitcoin-native NFT: a unique token with on-chain content and provenance that transfers by moving that specific satoshi. The architectural difference from Ethereum's NFTs is notable. Ethereum relies on smart contract standards like ERC-721, often with media stored off-chain on services like IPFS. Bitcoin achieves uniqueness through ordinal numbering, with the media bytes embedded directly in the blockchain's witness data. The result is a digital artifact whose uniqueness is enforced by Bitcoin's transaction model combined with off-chain indexers that follow ordinal conventions. Transferring inscriptions requires purpose-built wallets that carefully control which satoshis are spent, since standard wallets aren't aware of ordinal numbering.
