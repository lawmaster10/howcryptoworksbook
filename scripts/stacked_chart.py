#!/usr/bin/env python3
"""
Script to create stacked charts from Curve vs Uniswap data.
        ...
        if i + 3 < len(lines):
            ...
            if month and protocol and volume_str and color:
                try:
                    ...
                    # print(f"Parsed: {month}, {protocol}, {volume}, {color}")  # Comment out for cleaner output
...
def create_chart():
    """Create a stacked area chart from the input.txt data"""
    # Load data
    df = load_data()
    ...
    df['volume_billions'] = df['monthly_volume'] / 1e9
    
    # Pivot the data for stacked chart
    pivot_df = df.pivot(index='month', columns='protocol', values='volume_billions')
    ...
    ax.stackplot(pivot_df.index, 
                 *[pivot_df[col] for col in pivot_df.columns],
                 ...
                 colors=[colors[col] for col in pivot_df.columns],
                 alpha=0.8)
    
    # Customize the chart
    ax.set_title('Uniswap vs. Curve in USDT-USDC swaps\n(Monthly Trading Volume)', 
                 fontsize=16, fontweight='bold', pad=20)
    ax.set_xlabel('Month', fontsize=12, fontweight='bold')
    ax.set_ylabel('Trading Volume (Billions USD)', fontsize=12, fontweight='bold')
    ...
    # Add total volume statistics
    total_curve = df[df['protocol'] == 'Curve (USDT-USDC)']['monthly_volume'].sum()
    total_uniswap = df[df['protocol'] == 'Uniswap (USDT-USDC)']['monthly_volume'].sum()
    
    stats_text = f"Total Volume:\nCurve: ${total_curve/1e9:.1f}B\nUniswap: ${total_uniswap/1e9:.1f}B"
    ...
    # Save the chart
    output_file = '/Users/lawmaster/Desktop/Code/crypto 201/scripts/curve_vs_uniswap_chart.png'
...
if __name__ == "__main__":
    print("Creating stacked chart from input.txt...")
    create_chart()
    print("Done!")
